namespace OpenWeatherServicesApp.Services
{
    public class mainClass
    {
        internal string weatherUri = "http://api.openweathermap.org";
        internal string apiVersion = "/data/2.5/weather?";
        internal string firstParameterLatitude = "lat";

        internal string parameterLongitude = "lon";
        internal string parameterApiKey = "appid";
        internal string parametrUnitSystem = "units";

        internal string citiesListPath = "App_Data/city.list.json";
        internal string connectionStringPath = "connectionStrings.json";
        internal string connectionStringJson = "ConnectionStrings:weatherConnectionString";
        internal char humiditySymbol = '%';


        internal Dictionary<int, string> daysOfWeekPolish = new Dictionary<int, string>()
            {
                {1, "Poniedziałek"},
                {2, "Wtorek"},
                {3, "Środa"},
        ...
        internal Dictionary<Func<int, bool>, string> windDirectionsDesc = new Dictionary<Func<int, bool>, string>()
            {
                { item => item == 0, "cisza" },
